...
export function getIframeRE(): RegExp {
  return RegExp(/^<iframe.*?src="(.+?)".*?>.*?<\/iframe>$/.source, "i");
}
...
export function getListRE(): RegExp {
  return RegExp(/^(\s*)([*+-] \[[x ]\]\s|[*+-]\s|(\d+)([.)]\s))(\s*)/.source);
}
...
export function getMarkDownFileRE(): RegExp {
  return RegExp(/.+\.(?:md|markdown|txt|rmd)$/.source, "i");
}
